...
class DetailedLatencyMeasurement:
    ...
    t1: float         # زمان ارسال REQUEST از Master
    t2: float       # زمان دریافت REQUEST در Slave
    t3: float          # زمان ارسال RESPONSE از Slave
    t4: float      # زمان دریافت RESPONSE در Master
...
class P900NetworkTesterEnhanced:
    ...
    def __init__(self, local_port: str, remote_port: str, baudrate: int = 57600):
        ...
        self.mavlink_payload_enabled = False  # استفاده از MAVLink payload
                # Random size mode variables
        self.random_size_enabled = False
        self.random_size_distribution = 'realistic'
        self.random_size_min = 28
        self.random_size_max = 279
        self.random_sizes_queue = queue.Queue()
        self.packet_size_history = []  # ثبت اندازه همه پکت‌های ارسالی
    def _get_relative_time(self) -> float:
        """Get time relative to reference (in seconds)"""
    ...
    def set_packet_size(self, size: int) -> bool:
        ...
        return True
    def generate_random_packet_sizes(self, count: int = 100,
                                    min_size: int = 28,
                                    max_size: int = 279,
                                    distribution: str = 'uniform') -> List[int]:
        """
        تولید لیست رندوم اندازه پکت‌ها برای شبیه‌سازی ترافیک واقعی

        Args:
            count: تعداد اندازه‌های مورد نیاز
            min_size: حداقل اندازه پکت (بایت)
            max_size: حداکثر اندازه پکت (بایت)
            distribution: نوع توزیع ('uniform', 'normal', 'bimodal', 'realistic')
        ...
        if distribution == 'uniform':
            # توزیع یکنواخت
            sizes = np.random.randint(min_size, max_size + 1, size=count).tolist()

        elif distribution == 'normal':
            # توزیع نرمال (گاوسی) با میانگین در وسط رنج
            mean = (min_size + max_size) / 2
            std = (max_size - min_size) / 6  # 99.7% در رنج
            sizes = np.random.normal(mean, std, size=count)
            sizes = np.clip(sizes, min_size, max_size).astype(int).tolist()

        elif distribution == 'bimodal':
            # ✅ فقط یکبار تعریف شده (حذف تکرار)
            # توزیع دو قله‌ای (پکت‌های کوچک و بزرگ)
            for _ in range(count):
                if np.random.random() < 0.6:  # 60% پکت‌های کوچک
                    size = np.random.randint(min_size, min_size + 50)
                else:  # 40% پکت‌های بزرگ
                    size = np.random.randint(max_size - 50, max_size + 1)
                sizes.append(size)

        elif distribution == 'realistic':
            # توزیع واقعی بر اساس ترافیک MAVLink معمولی
            # اندازه‌های رایج MAVLink: 8-12 (heartbeat), 28-36 (attitude),
            # 52-64 (position), 100-120 (mission), 250+ (log/param)
            mavlink_common_sizes = {
                range(8, 13): 0.25,      # Heartbeat, status (25%)
                range(28, 37): 0.30,     # Attitude, GPS (30%)
                range(52, 65): 0.20,     # Position, velocity (20%)
                range(100, 121): 0.15,   # Mission, waypoint (15%)
                range(250, 280): 0.10    # Log, parameter (10%)
            }

            for _ in range(count):
                rand = np.random.random()
                cumulative = 0
                for size_range, prob in mavlink_common_sizes.items():
                    cumulative += prob
                    if rand < cumulative:
                        # انتخاب یک اندازه تصادفی از رنج
                        sizes.append(np.random.choice(list(size_range)))
                        break
                
                # در صورتی که به هر دلیلی انتخاب نشد، یک اندازه پیش‌فرض
                if len(sizes) < _ + 1:
                    sizes.append(np.random.randint(min_size, max_size + 1))

        else:
            # Fallback به توزیع uniform در صورت نامعتبر بودن نوع
            self.logger.warning(f"Unknown distribution '{distribution}', using uniform")
            sizes = np.random.randint(min_size, max_size + 1, size=count).tolist()

        # نمایش آمار تولید شده
        self.logger.info(f"Generated {count} random packet sizes:")
        self.logger.info(f"  Distribution: {distribution}")
        self.logger.info(f"  Min: {min(sizes)} bytes")
        self.logger.info(f"  Max: {max(sizes)} bytes")
        self.logger.info(f"  Mean: {np.mean(sizes):.1f} bytes")
        self.logger.info(f"  Std: {np.std(sizes):.1f} bytes")
        
        # نمایش هیستوگرام توزیع در لاگ (اختیاری)
        if distribution in ['bimodal', 'realistic']:
            hist, bins = np.histogram(sizes, bins=10)
            self.logger.debug(f"  Distribution histogram: {hist}")

        return sizes


    def set_random_size_mode(self, enabled: bool = True, 
                            distribution: str = 'realistic',
                            min_size: int = 28,
                            max_size: int = 279):
        """
        فعال/غیرفعال کردن حالت اندازه رندوم
        
        Args:
            enabled: فعال بودن حالت رندوم
            distribution: نوع توزیع اندازه‌ها
            min_size: حداقل اندازه
            max_size: حداکثر اندازه
        """
        self.random_size_enabled = enabled
        self.random_size_distribution = distribution
        self.random_size_min = min_size
        self.random_size_max = max_size
        
        if enabled:
            self.logger.info(f"✅ Random size mode ENABLED (distribution: {distribution})")
        else:
            self.logger.info("❌ Random size mode DISABLED")
    def generate_mavlink_payload(self, size: int, message_type: str = 'auto') -> bytes:
        """
        تولید payload شبیه پیام‌های واقعی MAVLink
        
        Args:
            size: اندازه payload مورد نیاز
            message_type: نوع پیام ('heartbeat', 'attitude', 'gps', 'auto')
        
        Returns:
            payload با ساختار شبیه MAVLink
        """
        if size < 1:
            return b''
        
        # MAVLink packet structure simulation
        payload = bytearray()
        
        # تعیین نوع پیام بر اساس اندازه (اگر auto باشه)
        ...
        if message_type == 'heartbeat':
            ...
            import random
            payload.extend(struct.pack('<Iffffff',
                                    int(time.time() * 1000) % (2**32),  # time_boot_ms
                                    random.uniform(-0.5, 0.5),          # roll
                                    random.uniform(-0.3, 0.3),          # pitch
                                    random.uniform(-3.14, 3.14),        # yaw
                                    random.uniform(-0.1, 0.1),          # rollspeed
                                    random.uniform(-0.1, 0.1),          # pitchspeed
                                    random.uniform(-0.1, 0.1)))         # yawspeed
        
        elif message_type == 'gps':
            # GPS: time_usec, lat, lon, alt, eph, epv, vel, cog, satellites
            payload.extend(struct.pack('<QiiiiHHHHB',
                                    int(time.time() * 1e6) % (2**64),  # time_usec
                                    357405500,   # lat (35.7405500)
                                    514265800,   # lon (51.4265800)
                                    1500000,     # alt (1500m)
                                    100,         # eph
                                    150,         # epv
                                    500,         # vel
        ...
        # پر کردن باقی فضا با pattern داده واقعی
        remaining = size - len(payload)
        if remaining > 0:
            # الگوی داده تلمتری
            pattern = bytes([0x00, 0xFF, 0xAA, 0x55, 0x12, 0x34, 0x56, 0x78])
            full_patterns = remaining // len(pattern)
            partial = remaining % len(pattern)
            
            for _ in range(full_patterns):
                payload.extend(pattern)
            if partial:
                payload.extend(pattern[:partial])
        
        return bytes(payload[:size])

    def use_mavlink_payload(self, enabled: bool = True):
        """
        فعال/غیرفعال کردن استفاده از MAVLink payload
        
        Args:
            enabled: استفاده از MAVLink payload به جای داده تصادفی
        """
        self.mavlink_payload_enabled = enabled
        if enabled:
            self.logger.info("✅ MAVLink payload mode ENABLED")
        else:
            self.logger.info("❌ MAVLink payload mode DISABLED")
    ...
    def start_services(self) -> bool:
        """شروع thread های Slave و Master Receiver"""
        self.logger.info("🚀 Starting required test services (Slave & Receiver)...")
        
        self.running = True
        
        # Initialize request timestamp tracking
        self.request_timestamps = {}
        
        # پاک کردن صف‌های قبلی
        ...
        self.master_receiver_thread = threading.Thread(target=self._master_receiver_loop, daemon=True)
        self.master_receiver_thread.start()
        
        # Wait briefly for services to be ready
        time.sleep(0.2)
        
        if self.slave_thread.is_alive() and self.master_receiver_thread.is_alive():
            self.logger.info("✅ Services started successfully.")
            return True
        else:
            self.logger.error("❌ Failed to start one or more critical services.")
            self.stop_services()
            return False

    def stop_services(self):
        """توقف thread ها (برای فراخوانی در انتهای تست)"""
        if not self.running:
            return
        ...
        if hasattr(self, 'slave_thread') and self.slave_thread.is_alive():
            self.slave_thread.join(timeout=1.0)
            
        if hasattr(self, 'master_receiver_thread') and self.master_receiver_thread.is_alive():
            self.master_receiver_thread.join(timeout=1.0)
            
        # Flush queue to prevent blocking on subsequent calls
        while not self.response_queue.empty():
            try:
                self.response_queue.get_nowait()
            except queue.Empty:
        ...
        # پاک کردن شمارنده‌ها برای تست بعدی
        self.requests_sent = 0
        self.requests_received = 0
        self.responses_sent = 0
        self.responses_received = 0
    ...
    def _print_summary(self):
        """نمایش خلاصه نتایج تست"""
        total = len(self.measurements)
        successful = len([m for m in self.measurements if m.status == "OK"])
        timeouts = len([m for m in self.measurements if m.status == "TIMEOUT"])
        
        self.logger.info("="*60)
        self.logger.info("📊 Test Summary:")
        self.logger.info(f"   Total packets: {total}")
        ...
        if successful > 0:
            ...
            self.logger.info(f"\n📈 RTT Statistics (ms):")
            self.logger.info(f"   Mean: {np.mean(rtts):.2f}")
            self.logger.info(f"   Median: {np.median(rtts):.2f}")
            self.logger.info(f"   Min: {np.min(rtts):.2f}")
            self.logger.info(f"   Max: {np.max(rtts):.2f}")
            self.logger.info(f"   Std Dev: {np.std(rtts):.2f}")
            
            if len(rtts) > 1:
                self.logger.info(f"   P95: {np.percentile(rtts, 95):.2f}")
                self.logger.info(f"   P99: {np.percentile(rtts, 99):.2f}")
    ...
    def _create_packet(self, packet_id: int, packet_type: int,
        ...
        t1_us = int(t1 * 1_000_000) & 0xFFFFFFFF
        t2_us = int(t2 * 1_000_000) & 0xFFFFFFFF
        t3_us = int(t3 * 1_000_000) & 0xFFFFFFFF
        ...
        if self.mavlink_payload_enabled:
            payload = self.generate_mavlink_payload(self.payload_size)
        else:
    ...
    def _parse_packet(self, data: bytes) -> Optional[Dict]:
        ...
        try:
            ...
            # ✅ اصلاح شده: حفظ timestamps به صورت relative seconds
            # timestamps در packet به صورت microseconds ذخیره شده‌اند
            t1_us = struct.unpack('<I', data[7:11])[0]
            t2_us = struct.unpack('<I', data[11:15])[0]
            t3_us = struct.unpack('<I', data[15:19])[0]
            
            # تبدیل از microseconds به seconds (relative time)
            t1_seconds = t1_us / 1_000_000.0 if t1_us > 0 else 0
            t2_seconds = t2_us / 1_000_000.0 if t2_us > 0 else 0
            t3_seconds = t3_us / 1_000_000.0 if t3_us > 0 else 0
            ...
            return {
                'packet_id': packet_id,
                'packet_type': packet_type,  # ✅ این عدد است، نه رشته
                't1': t1_seconds,
                't2': t2_seconds,
                't3': t3_seconds
            }
    ...
    def _slave_loop(self):
        ...
        while self.running:
            try:
                if self.remote_serial and self.remote_serial.in_waiting > 0:
                    ...
                    while len(buffer) >= self.total_packet_size:
                        ...
                        if len(buffer) >= self.total_packet_size:
                            ...
                            t2 = self._get_relative_time()

                            packet_data = bytes(buffer[:self.total_packet_size])
                            packet = self._parse_packet(packet_data)

                            # ✅ اصلاح شده: مقایسه با عدد به جای رشته
                            if packet and packet['packet_type'] == self.TYPE_REQUEST:
                ...
                if self.running:
                    self.logger.error(f"   ❌ Slave error: {e}", exc_info=True)  # ✅ اضافه شد exc_info
                    time.sleep(0.01)
    ...
    def measure_latency(self, num_packets: int = 100, interval_ms: float = 100) -> List[DetailedLatencyMeasurement]:
        ...
        # ✅ اگر random mode فعال است، لیست اندازه‌ها را تولید کن
        if self.random_size_enabled:
            sizes = self.generate_random_packet_sizes(
                count=num_packets,
                min_size=self.random_size_min,
                max_size=self.random_size_max,
                distribution=self.random_size_distribution
            )
            # اضافه کردن به صف
            for size in sizes:
                self.random_sizes_queue.put(size)
            self.logger.info(f"✅ Generated {num_packets} random packet sizes")
        
        self.measurements.clear()
        self.packet_size_history.clear()  # ✅ پاک کردن تاریخچه قبلی
        
        # پاک کردن صف response قبلی
        while not self.response_queue.empty():
            try:
                self.response_queue.get_nowait()
            except:
        ...
        # شروع services اگر در حال اجرا نیستند
        if not self.running:
        ...
        for packet_id in range(num_packets):
            # ✅ تنظیم اندازه پکت برای این ارسال
            if self.random_size_enabled and not self.random_sizes_queue.empty():
                new_size = self.random_sizes_queue.get()
                self.set_packet_size(new_size)
                self.packet_size_history.append(new_size)
            else:
                self.packet_size_history.append(self.total_packet_size)
            
            current_packet_size = self.total_packet_size  # ✅ ذخیره اندازه فعلی
            
            # ثبت زمان ارسال REQUEST (T1)
            t1 = self._get_relative_time()
            
            # ذخیره T1 برای این packet_id
            self.request_timestamps[packet_id] = t1
            
            # ساخت REQUEST
            request = self._create_packet(packet_id, self.TYPE_REQUEST, t1=t1)
            
            # پاک کردن صف response قبل از ارسال جدید
            while not self.response_queue.empty():
                try:
                    self.response_queue.get_nowait()
                except:
            ...
            self.logger.debug(f"📤 Master: Sent REQUEST #{packet_id} (size: {current_packet_size} bytes)")
            ...
            try:
                ...
                if response_data['packet_id'] == packet_id:
                    # محاسبه تأخیرها
                    forward_ms = (response_data['t2'] - response_data['t1']) * 1000
                    return_ms = (response_data['t4'] - response_data['t3']) * 1000
                    rtt_ms = (response_data['t4'] - response_data['t1']) * 1000
                    processing_ms = (response_data['t3'] - response_data['t2']) * 1000
                    asymmetry_ms = forward_ms - return_ms
                    
                    measurement = DetailedLatencyMeasurement(
                        packet_id=packet_id,
                        packet_size=current_packet_size,  # ✅ اندازه صحیح ذخیره می‌شود
                        t1=response_data['t1'],
                        t2=response_data['t2'],
                        t3=response_data['t3'],
                        t4=response_data['t4'],
                        ...
                        asymmetry_ms=asymmetry_ms,
                        status="OK"
                    )
                    response_received = True
                    
                    # ✅ Validation برای timestamps
                    if t1 > response_data['t2'] or response_data['t2'] > response_data['t3'] or response_data['t3'] > response_data['t4']:
                        self.logger.warning(f"⚠️ Invalid timestamp sequence for packet {packet_id}")
                        measurement.status = "INVALID_TIMESTAMP"
                    
                else:
                    measurement = DetailedLatencyMeasurement(
                        packet_id=packet_id,
                        packet_size=current_packet_size,
                        t1=t1, t2=0, t3=0, t4=0,
                        forward_delay_ms=0, return_delay_ms=0, rtt_ms=0,
                        processing_time_ms=0, asymmetry_ms=0,
                        status="MISMATCH"
                    )
                    self.logger.warning(f"⚠️ Packet ID mismatch: expected {packet_id}, got {response_data['packet_id']}")
                ...
                self.logger.warning(f"⏱️ Timeout for packet {packet_id}")
    ...
    def plot_packet_size_distribution(self):
        """نمایش توزیع اندازه‌های پکت استفاده شده"""
        if not self.packet_size_history:
            self.logger.warning("No packet size history available")
            return
        ...
        # هیستوگرام
        plt.subplot(1, 2, 1)
        plt.hist(self.packet_size_history, bins=20, edgecolor='black', alpha=0.7)
        ...
        # Box plot
        plt.subplot(1, 2, 2)
        plt.boxplot(self.packet_size_history)
        plt.ylabel('Packet Size (bytes)')
        plt.title('Packet Size Statistics')
        plt.grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(f'packet_size_dist_{get_timestamp()}.png')
        plt.show()


    def disconnect(self):
        """قطع اتصال از پورت‌های سریال"""
        self.logger.info("🔌 Disconnecting serial ports...")
        
        # توقف services اگر در حال اجرا هستند
        if self.running:
            self.stop_services()
        ...
        if self.local_serial and self.local_serial.is_open:
            try:
                ...
                self.logger.info(f"✅ Disconnected from Master port: {self.local_port}")
            except Exception as e:
                self.logger.error(f"❌ Error closing Master port: {e}")
        
        # بستن پورت Slave
        if self.remote_serial and self.remote_serial.is_open:
            try:
                self.remote_serial.close()
                self.logger.info(f"✅ Disconnected from Slave port: {self.remote_port}")
            except Exception as e:
                self.logger.error(f"❌ Error closing Slave port: {e}")
        
        self.local_serial = None
        self.remote_serial = None
        self.logger.info("✅ All ports disconnected")
    ...
    def plot_results(self, output_prefix: str = None) -> str:
            ...
            if self.packet_size_history:  # اگر اندازه‌های متغیر داریم
                ax1_twin = ax1.twinx()  # محور y دوم
                ax1_twin.plot(packet_ids[:len(self.packet_size_history)], 
                            self.packet_size_history, 
                            'g--', alpha=0.5, label='Packet Size')
                ax1_twin.set_ylabel('Packet Size (bytes)', color='g')
    ...
    def get_mavlink_message_distribution(self) -> Dict[str, int]:
        """
        آمار توزیع انواع پیام‌های MAVLink بر اساس اندازه
        """
        if not self.packet_size_history:
            return {}
        ...
        for size in self.packet_size_history:
            payload_size = size - self.HEADER_SIZE
            if payload_size <= 12:
                distribution['heartbeat'] += 1
            elif payload_size <= 36:
                distribution['attitude'] += 1
            elif payload_size <= 64:
                distribution['gps'] += 1
            else:
                distribution['telemetry'] += 1
